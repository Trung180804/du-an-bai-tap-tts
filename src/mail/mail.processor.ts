...
import { MailJob, MailSubject, MailTemplate, QueueName } from '@/common/enums';
...
export class MailProcessor {
  ...
  @Process(MailJob.SEND_WELCOME_EMAIL)
  async handleSendWelcomeEmail(job: Job) {
    ...
    const logoBase64 = fs.existsSync(logoPath)
      ...
      : '';
    const placeholderBase64 = fs.existsSync(placeholderPath)
      ? fs.readFileSync(placeholderPath, 'base64')
      : '';

    const templateName = MailTemplate.WELCOME;
    ...
    if (fs.existsSync(templatePath)) {
      ...
      source = fs.readFileSync(
        join(rootDir, 'src', 'templates', 'welcome.vi.hbs'),
        'utf8',
      );
    }
    ...
    const subject =
      lang === 'en' ? MailSubject.WELCOME_EN : MailSubject.WELCOME_VI;
